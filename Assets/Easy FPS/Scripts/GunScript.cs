...
public class GunScript : MonoBehaviour {
	...
	*/
	...
	void Sprint(){// Running();  so i can find it with CTRL + F
		pmS.maxSpeed = Input.GetKey(KeyCode.LeftShift) && !aiming? runningSpeed: walkingSpeed;
	...
	*/
	...
	void DrawCrosshair(){
		...
		if(Input.GetAxis("Fire2") == 0){//if not aiming draw
			...
			var w = Screen.width;
			var h = Screen.height;
			var e = position_y(expandValues_crosshair.y) + w/100;
			var ww = w/20;
			var hh = w/100;
			GUI.DrawTexture(new Rect(w/2 - hh/2, h/2 - ww/2 - e, hh, ww), horizontal_crosshair); //top
			GUI.DrawTexture(new Rect(w/2 - hh/2, h/2 - ww/2 + e, hh, ww), horizontal_crosshair); //bottom
			GUI.DrawTexture(new Rect(w/2 - ww/2 - e, h/2 - hh/2, ww, hh), vertical_crosshair); //left
			GUI.DrawTexture(new Rect(w/2 - ww/2 + e, h/2 - hh/2, ww, hh), vertical_crosshair); //right
		}
